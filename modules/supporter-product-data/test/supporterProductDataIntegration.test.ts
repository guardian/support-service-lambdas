...
describe('Supporter Product Data Integration Tests', () => {
	test('getSupporterProductData Integration', async () => {
		const supporterData = await getSupporterProductData('CODE', '110001137');
		expect(supporterData?.length).toEqual(4);
	});

	test('getSubscriptionDetails Integration', async () => {
		const subscription = await getSubscription('CODE', 'A-S00610788');
