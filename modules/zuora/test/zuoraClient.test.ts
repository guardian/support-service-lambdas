...
describe('ZuoraClient fetch method error handling', () => {
	...
	describe('HTTP 400 errors with different response formats', () => {
		test('should handle /v1/object/* error format with Errors array', async () => {
			...
			// Current implementation throws generic error, but we want detailed error info
			try {
	...
	// describe('HTTP 200 responses with success: false (critical issue)', () => {
	// 	test('should handle account not found error (HTTP 200 but success: false)', async () => {
	// 		const mockResponse = {
	// 			ok: true, // This is the problem - HTTP 200 but logical error
	// 			status: 200,
	// 			statusText: 'OK',
	// 			json: jest.fn().mockResolvedValue({
	// 				success: false,
	...
	// 		// This should throw an error, but current implementation will try to parse as success
	// 		await expect(
	// 			zuoraClient.fetch('v1/accounts/xxx/payment-methods', 'GET', testSchema),
	// 		).rejects.toThrow();

	// 		// Current implementation will fail here because it only checks response.ok
	// 		// and will try to parse the error response with the success schema
	// 	});
	...
	// 		await expect(
	// 			zuoraClient.fetch(
	// 				'v1/accounts/8ad09b7d83a313110183a8769afd1bf3/payment-methods1',
	// 				'GET',
	// 				testSchema,
	// 			),
	// 		).rejects.toThrow();
	...
	// 			// Current implementation logs headers but doesn't include retry info in error
	// 			// We want the error to include retry-after information
	...
	// test('should handle successful responses without success field (legacy format)', async () => {
	// 	const mockResponse = {
	// 		ok: true,
	// 		status: 200,
	// 		statusText: 'OK',
