...
object Query extends Enum[Query] {
  ...
  case object InvoiceItem extends Query(
    "InvoiceItem",
    "SELECT accountingCode, appliedToInvoiceItemId, balance, chargeAmount, chargeDate, chargeName, createdById, createdDate, invoice.Id, processingType, quantity, revRecStartDate, serviceEndDate, serviceStartDate, sku, product.Description, product.Id, product.Name, productRatePlanCharge.Id, quantity, ratePlanCharge.Id, revRecStartDate, serviceEndDate, serviceStartDate, sku, subscriptionId, taxAmount, taxCode, taxExemptAmount, taxMode, unitPrice, uom, updatedById, updatedDate, id, account.Id, accountingPeriod.Id, amendment.Id, billToContact.Id, billToContactSnapShot.Id, defaultPaymentMethod.Id, deferredRevenueAccountingCode.Id, invoice.Id, journalEntry.Id, journalRun.Id, parentAccount.Id, product.Id, productRatePlan.Id, productRatePlanCharge.Id, ratePlan.Id, ratePlanCharge.Id, soldToContact.Id, soldToContact.Id, subscription.Id FROM InvoiceItem  WHERE (CreatedDate >= '2019-08-28T00:00:00') AND (CreatedDate <= '2099-01-01T00:00:00')",
