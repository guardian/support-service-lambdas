...
object DigitalVoucherService {
  ...
  def apply[F[_]: Monad](imovoClient: ImovoClient[F]): DigitalVoucherService[F] = new DigitalVoucherService[F] {
    ...
    def toVoucher(
        voucherResponse: ImovoSubscriptionResponse,
    ): Either[DigitalVoucherServiceError, SubscriptionVouchers] = {
      (
        voucherResponse.subscriptionVouchers
          .find(_.subscriptionType === ImovoSubscriptionType.ActiveLetter.value)
          .toRight(
            List("Imovo response did not contain an subscription voucher where subscriptionType==\"ActiveLetter\" "),
          ),
        voucherResponse.subscriptionVouchers
          .find(_.subscriptionType === ImovoSubscriptionType.ActiveCard.value)
          .toRight(
            List("Imovo response did not contain an subscription voucher where subscriptionType==\"ActiveCard\" "),
          ),
    ...
    def toReplacementVoucher(
        voucherResponse: ImovoSubscriptionResponse,
    ): Either[DigitalVoucherServiceError, ReplacementSubscriptionVouchers] = {
      val cardCode =
        voucherResponse.subscriptionVouchers.find(_.subscriptionType === ImovoSubscriptionType.ActiveCard.value)
      val letterCode =
        voucherResponse.subscriptionVouchers.find(_.subscriptionType === ImovoSubscriptionType.ActiveLetter.value)

      (cardCode, letterCode) match {
        case (Some(card), Some(letter)) =>
          Right(ReplacementSubscriptionVouchers(Some(card.voucherCode), Some(letter.voucherCode)))
    ...
    ): EitherT[F, DigitalVoucherServiceError, RedemptionHistory] = {
      imovoClient
