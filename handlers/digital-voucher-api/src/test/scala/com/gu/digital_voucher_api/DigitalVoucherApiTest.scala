  ...
  "DigitalVoucherApi" should "return voucher details for create subscription request" in {
    ...
    val response = app
      .run(
        Request[IO](
          method = Method.PUT,
          uri = Uri(path = Uri.Path.unsafeFromString(s"/digital-voucher/${subscriptionId.value}")),
  ...
  it should "return voucher details for replace request with subscriptionId" in {
    ...
    val response = app
      .run(
        Request[IO](
          method = Method.POST,
          Uri(path = Uri.Path.unsafeFromString("/digital-voucher/replace")),
        ).withEntity[String](
  ...
  it should "return voucher details for get subscription request" in {
    ...
    val response = app
      .run(
        Request(
          method = Method.GET,
          Uri(path = Uri.Path.unsafeFromString(s"/digital-voucher/${subscriptionId.value}")),
        ),
      )
      .value
      .unsafeRunSync()
  ...
  it should "return 200 response for cancel request" in {
    ...
    val response = app
      .run(
        Request[IO](
          method = Method.POST,
          Uri(path = Uri.Path.unsafeFromString("/digital-voucher/cancel")),
        ).withEntity[String](
  ...
  it should "return 200 response for redemption history request with no redemption history" in {
    ...
    val response = app
      .run(
        Request(
          method = Method.GET,
          Uri(path = Uri.Path.unsafeFromString(s"/digital-voucher/redemption-history/${subscriptionId.value}")),
        ),
      )
      .value
      .unsafeRunSync()
