...
class SalesforceConnector(
    runRequest: HttpRequest => Either[ContactUsError, HttpResponse[String]]
) {
  ...
  def sendReq(
    ...
    runRequest(
      Http(env.reqEndpoint)
        ...
        if (response.isSuccess)
          decode[SFCompositeResponse](
            response.body,
            Some("SFCompositeResponse")
          ...
          decode[List[SFErrorDetails]](
            response.body,
            Some("List[SFErrorDetails]")
          ).flatMap(errors =>
              Left(
                ContactUsError(
                  "Salesforce",
