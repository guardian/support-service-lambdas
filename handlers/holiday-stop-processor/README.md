...
This lambda adds holiday stop credits to subscriptions using the Zuora Orders API.
...
1. writes back to Salesforce details of the credits added.

The requests fetched from Salesforce depend on the type of product that the request affects.  There's a set of rules for each publication.  It's a just-in-time process so that credits are only applied at the last moment before the fulfilment of that publication can be altered.  Every publication has a lead time for its fulfilment during which it cannot be altered: typically this is a period of 10 to 14 days.

There is a separate holiday credit applied in Zuora for each individual stopped publication date.
...
#### Zuora
It submits an asynchronous Order to add the credit to each relevant subscription in Zuora.
...
This is generated by a [ZOQL query](https://github.com/guardian/fulfilment-lambdas/blob/main/src/weekly/query.js#L114-L129) over the applied holiday credit charges.
