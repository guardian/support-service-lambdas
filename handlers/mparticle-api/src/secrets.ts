...
export async function getSecretValue(parameterStorePath: string, fallbackEnvVarKey?: string): Promise<string> {
    ...
    try {
        ...
        console.warn(`It was not possible to obtain secret value '${parameterStorePath}' from AWS Systems Manager Parameter Store: ${errorMessage}`);
    }

    if (!value && fallbackEnvVarKey) {
        // Fallback to Env vars
        console.warn(`Secret value '${parameterStorePath}' fallback to Env var value '${fallbackEnvVarKey}'.`);
