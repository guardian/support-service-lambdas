...
async function getProcessorDomainCertificate(
	...
	if (!cachedProcessorCertificate) {
		...
		const certificateResponse = await fetch(
			discoveryResponse.data.processor_certificate,
			{
				method: 'GET',
			},
		);
	...
	const now = new Date();
	const notExpired =
		now >= cachedProcessorCertificate.parsed.notBefore &&
		now <= cachedProcessorCertificate.parsed.notAfter;
	...
	if (!notExpired) {
		...
		console.error(
			'Certificate remains expired after refresh attempt. Please contact mParticle support for assistance.',
		);
		return null;
	}
...
export const validateDataSubjectRequestCallback = async (
	...
	if (
		!processorDomain ||
		!ALLOWED_PROCESSOR_DOMAINS.includes(processorDomain)
	) {
		console.error(
			'Invalid Processor Domain on Data Subject Request Callback validation.',
			processorDomain,
		);
		return false;
	}
	...
	if (certSubjectCN !== processorDomain) {
		console.error(
			`Certificate CN mismatch. Expected: ${processorDomain}, Found: ${certSubjectCN}`,
		);
		return false;
	}
