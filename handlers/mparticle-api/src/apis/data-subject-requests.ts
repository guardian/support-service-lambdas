...
export const submitDataSubjectRequest = async (form: DataSubjectRequestForm): Promise<DataSubjectRequestSubmission> => {
    const response = await requestDataSubjectApi<{
        ...
        body: {
            ...
            api_version: "3.0",
            status_callback_urls: getEnv('STAGE').toLowerCase() === "code" ? [
                `https://mparticle-api.code.membership.guardianapis.com/data-subject-requests/${form.requestId}/callback`
            ] : [
                `https://mparticle-api.membership.guardianapis.com/data-subject-requests/${form.requestId}/callback`
            ],
