...
class QueryZuoraSubscriptionsTest extends FlatSpec with Matchers {

  it should "build a valid query for a single account" in {
    val query = QueryZuoraSubscriptions.buildQuery(List(AccountId("acc123")))
  ...
  it should "build a valid query for multiple accounts" in {
    val query = QueryZuoraSubscriptions.buildQuery(List(AccountId("acc123"), AccountId("acc321"), AccountId("acc456")))
