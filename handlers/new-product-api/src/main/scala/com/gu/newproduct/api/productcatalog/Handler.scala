...
object Handler extends Logging {
  ...
  def runWithEffects(lambdaIO: LambdaIO, stage: Stage, fetchString: StringFromS3): ApiGatewayOp[Operation] = for {
    ...
    plansWithPrice = PricesFromZuoraCatalog(_: Stage, fetchString, zuoraToPlanId)
    catalog <- NewProductApi.catalog1(plansWithPrice, stage).toApiGatewayOp("load plan prices from zuora")
