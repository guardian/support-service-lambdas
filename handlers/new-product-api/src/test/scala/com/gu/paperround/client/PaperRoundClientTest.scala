...
class PaperRoundClientTest extends AnyFlatSpec with Matchers {
  ...
  "paperround client" should "get coverage successfully in test env" taggedAs EffectsTest in {
    val actual = for {
      client <- client
      coverage <- GetAgents(client).getAgents().toDisjunction
    } yield coverage
    println("actual: " + actual.getOrElse(throw new RuntimeException))
  }

  it should "get all agents successfully in test env" taggedAs EffectsTest in {
    val actual = for {
      client <- client
      coverage <- GetAgents(client).getAgents().toDisjunction
    } yield coverage
