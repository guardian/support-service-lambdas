...
class WindowRuleTest extends FlatSpec with Matchers {

  val wednesdayAugust8 = LocalDate.of(2018, 8, 8)

  def getWedAugust8 = () => wednesdayAugust8

  it should "define an unbounded window starting today if no params are defined" in {
    val rule = WindowRule(
      now = getWedAugust8,
      maybeCutOffDay = None,
    ...
    rule.selectableWindow shouldBe SelectableWindow(wednesdayAugust8, None)
  }
  ...
  "windowRule" should "define an unbounded window starting tomorrow if tomorrow is the cut off day and there are no other parameters defined " in {
    ...
    rule.selectableWindow shouldBe SelectableWindow(LocalDate.of(2018, 8, 9), None)
  }
  ...
  it should "define an unbounded window starting cut off day next week if the cut off date was yesterday" in {
    ...
    rule.selectableWindow shouldBe SelectableWindow(LocalDate.of(2018, 8, 14), None)
  }
  ...
  it should "define an unbounded window starting in two days if only the start delay is set to 2" in {
    ...
    rule.selectableWindow shouldBe SelectableWindow(wednesdayAugust8.plusDays(2), None)
  }
  ...
  it should "define a 2 day window starting today if only the window size is set to 2 days" in {
    ...
    rule.selectableWindow shouldBe SelectableWindow(wednesdayAugust8, Some(wednesdayAugust8.plusDays(2)))
  }
  ...
  it should "define a 2 day window starting next week 3 days after cut off date" in {
    ...
    rule.selectableWindow shouldBe SelectableWindow(expectedWindowStart, Some(expectedWindowEnd))
  }
}
