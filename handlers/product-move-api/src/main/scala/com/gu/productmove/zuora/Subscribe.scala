...
private class SubscribeLive(zuoraGet: ZuoraGet) extends Subscribe :
  override def create(zuoraAccountId: String, targetProductId: String): ZIO[Stage, String, CreateSubscriptionResponse] = {
    for {
...
object Subscribe {
  def create(zuoraAccountId: String, targetProductId: String): ZIO[Subscribe with Stage, String, CreateSubscriptionResponse] =
...
object SubscribeRequest {
  ...
  def withTodaysDate(zuoraAccountId: String, targetProductId: String): ZIO[Stage, Nothing, SubscribeRequest] =
    for {
      ...
      percentageDiscountRatePlanId = if (stage == Stage.PROD) "2c92a0ff5345f9220153559d915d5c26" else "2c92c0f85721ff7c01572942235b6d7a"
      percentageDiscountRatePlanChargeId = if (stage == Stage.PROD) "2c92a0fd5345efa10153559e97bb76c6" else "2c92c0f957220b5d0157299c97a60bbd"

      // discount hardcoded for MVP (50% off for 3 Months)
      discount = SubscribeToRatePlans(productRatePlanId = percentageDiscountRatePlanId, List(ChargeOverrides(productRatePlanChargeId = percentageDiscountRatePlanChargeId, discountPercentage = Some(50), upToPeriods = Some(3), endDateCondition = Some("Fixed_Period"))))
      ...
      subscribeToRatePlans = List(SubscribeToRatePlans(productRatePlanId = targetProductId), discount),   // hardcode 50% offer for 3 months
