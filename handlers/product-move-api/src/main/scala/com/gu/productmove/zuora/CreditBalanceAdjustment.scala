...
private class CreditBalanceAdjustmentLive(zuoraGet: ZuoraGet) extends CreditBalanceAdjustment :
  override def adjust(amount: BigDecimal, comment: String, invoiceId: String, `type`: String): IO[String, Res] = {
    ...
    zuoraGet.post[RequestBody, Res](uri"object/credit-balance-adjustment", body, ZuoraSuccessCheck.SuccessCheckCapitalised)
  }

trait CreditBalanceAdjustment:
  def adjust(amount: BigDecimal, comment: String, invoiceId: String, `type`: String): ZIO[CreditBalanceAdjustment, String, Res]
