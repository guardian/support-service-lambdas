...
import com.gu.productmove.zuora.{CreditBalanceAdjustment, GetAccountLive, GetInvoiceToBeRefunded, GetSubscriptionLive, SubscribeLive, ZuoraCancel, ZuoraCancelLive, ZuoraSetCancellationReason}
...
object Refund {
  def applyRefund(refundInput: RefundInput): ZIO[
    InvoicingApiRefund with CreditBalanceAdjustment with Stage with SttpBackend[Task, Any] with AwsS3 with GetInvoiceToBeRefunded,
    String,
    Unit,
  ] = {
    ...
    for {
      ...
      _ <- ZIO.log(s"Amount to be refunded is $amount")
