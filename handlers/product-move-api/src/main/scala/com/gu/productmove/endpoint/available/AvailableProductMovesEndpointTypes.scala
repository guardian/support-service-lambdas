...
object MoveToProduct {
  ...
  def buildResponseFromRatePlan(
    ...
    for {
      ...
      pricing <- ZIO
        .fromOption(productRatePlan.productRatePlanCharges.head.pricing.find(_.currency == "GBP"))
        .orElse(ZIO.log(s"currency not found on ratePlanCharge").flatMap(_ => ZIO.fail(AvailableMoves(List()))))
      ...
      introOffer = Offer(
        Billing(
          ...
          currency = None,
          frequency = None,
          startDate = Some(localDateToString.format(chargedThroughDate)),
        ),
        TimePeriod(TimeUnit.month, 3),
      )
      newPlan = Billing(
        amount = Some(price.toInt),
        percentage = None,
        currency = Some(GBP),
        frequency = Some(TimePeriod(TimeUnit.fromString(billingPeriod), 1)),
        startDate = Some(localDateToString.format(chargedThroughDate.plusDays(90))),
      )
    } yield MoveToProduct(
      id = productRatePlan.id,
      name = "Digital Pack",
      trial = Some(Trial(dayCount = 14)),
      introOffer = Some(introOffer),
      billing = newPlan,
    )
}
...
object Currency {
  ...
  given JsonCodec[Currency] = JsonCodec[Map[String, String]].transformOrFail[Currency](
    _.get("code")
      .toRight("no code in currency object")
      .flatMap(code => Try(Currency.valueOf(code)).toEither.left.map(_.toString)),
    ...
    Schema[Currency](
      SchemaType.SProduct[Currency](
        List(
          SProductField(
            FieldName("code"),
            Schema(SString(), encodedExample = Some("GBP"), description = Some("ISO 4217 alphabetic currency code.")),
            (c: Currency) => None,
          ),
          SProductField(
            FieldName("symbol"),
            Schema(SString(), encodedExample = Some("£"), description = Some("ISO 4217 currency symbol.")),
            (c: Currency) => None,
          ),
        ),
      ),
