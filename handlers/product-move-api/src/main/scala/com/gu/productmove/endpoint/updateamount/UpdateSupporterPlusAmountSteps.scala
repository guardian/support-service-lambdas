  ...
  private def getSupporterPlusData(
    ...
    for {
      ...
      chargesById <- {
        ...
        if (errors.nonEmpty) ZIO.fail(BadRequest("subscription had duplicate charges")).logError(s"errors: $errors")
      ...
      supporterPlusCharges <- chargeIdsOfInterest.map(chargesById.get) match
        ...
        case other => ZIO.fail(BadRequest("subscription was not in valid state")).logError(s"charges: $other")
