  ...
  override def spec: Spec[TestEnvironment & Scope, Any] =
    suite("RunBilling")(
      test("serialisers - failure case") {
        val testData =
          """[{"Errors":[{"Message":"The invoice will not be generated because there are no charges due on this account.","Code":"INVALID_VALUE"}],"Success":false}]"""
        val expected = Left(
          RunBillingErrorResponse(
            List(
              ZuoraError(
                "INVALID_VALUE",
                "The invoice will not be generated because there are no charges due on this account.",
              ),
            ),
          ),
        )
