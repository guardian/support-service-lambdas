...
object SalesforceDDMandate extends Logging {
  ...
  object GetAllPaymentMethodWithBillingAccountGivenGoCardlessReference {
    ...
    def apply(sfGet: HttpOp[RestRequestMaker.GetRequest, JsValue]): List[Reference] => ClientFailableOp[Map[Reference, SfPaymentMethodDetail]] =
    ...
    def toRequest(references: List[Reference]) = {
      val soqlQuery = s"SELECT Id, Zuora__MandateID__c, Zuora__BillingAccount__c, Zuora__BankTransferAccountNumber__c " +
    ...
    def toResponse(referenceSearchQueryResponse: ReferenceSearchQueryResponse): Map[Reference, SfPaymentMethodDetail] =
      referenceSearchQueryResponse.records.map(paymentMethodDetail =>
        paymentMethodDetail.Zuora__MandateID__c -> paymentMethodDetail).toMap
