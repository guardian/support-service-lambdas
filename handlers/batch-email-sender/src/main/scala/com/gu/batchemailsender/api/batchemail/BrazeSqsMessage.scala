...
object BrazeSqsMessage {
  ...
  def fromSalesforceMessage(salesforceBatchItem: SalesforceBatchItem): BrazeSqsMessage = {
    ...
    val emailPayloadTo = EmailPayloadTo(
      ...
      ContactAttributes = EmailPayloadContactAttributes(
        SubscriberAttributes = BrazeApiTriggerProperties(
          ...
          delivery_problem_affected_dates = salesforcePayload.delivery_problem.map(_.deliveries.map(d => fromSfDateToDisplayDate(d.Delivery_Date__c)).mkString(", ")),
          delivery_problem_total_credit = salesforcePayload.delivery_problem.map(_.totalCreditAmount),
          delivery_problem_invoice_date = salesforcePayload.delivery_problem.flatMap(_.deliveries.flatMap(_.Invoice_Date__c).headOption.map(fromSfDateToDisplayDate)),
          ...
          delivery_address_change_line1 = salesforcePayload.delivery_address_change.flatMap(_.mailingStreet).flatMap(sfStreetToLine1),
          delivery_address_change_line2 = salesforcePayload.delivery_address_change.flatMap(_.mailingStreet).flatMap(sfStreetToLine2),
          ...
          delivery_address_change_effective_date_blurb = salesforcePayload.delivery_address_change.flatMap(_.addressChangeEffectiveDateBlurb),
        )
      )
    )
    ...
    BrazeSqsMessage(
      To = emailPayloadTo,
      DataExtensionName = brazeCampaignId(salesforceBatchItem),
  ...
  def brazeCampaignId(salesforceBatchItem: SalesforceBatchItem): String =
