...
class PartitionSchedules(
  ...
  def partition(schedules: List[(RevenueScheduleAquaRow, GetSubscription.GetSubscriptionResponse)]): (List[DistributeToday], List[DistributeRange]) = {
      ...
      case (next, (todaySoFar, rangeSoFar)) =>
        next match {
          ...
          case (
            ...
            GetSubscription.GetSubscriptionResponse(Some(redemptionDate), /* isRedeemed = */ true, termStartDate, initialTerm, initialTermPeriodType))
          =>
            if (initialTermPeriodType == "Day") {
              ...
              val newRecord = DistributeRange(refundSchedule, redemptionDate, termStartDate.plusDays(initialTerm - 1)) // recognition end is inclusive
