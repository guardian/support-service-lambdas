...
import com.gu.sf_datalake_export.salesforce_bulk_api.{DownloadResult, GetBatchResultId, GetJobBatches}
...
object DownloadBatch {
  ...
  case class WireRequest(jobId: String, batchId: String)
  ...
  def steps(
    ...
    val lambdaResponse = for {
      ...
      - <- downloadresultOp(downloadRequest).toDisjunction.leftMap(failure => failure.message)
    } yield ()
