...
export interface TestApiProps extends GuStackProps {
	stack: string;
	stage: string;
...
export class TestApi extends GuStack {
	constructor(scope: App, id: string, props: TestApiProps) {
		...
		const usagePlan = lambda.api.addUsagePlan('UsagePlan', {
			name: nameWithStage,
			description: 'REST endpoints for test-api>',
			apiStages: [
				{
		...
		const alarmName = (shortDescription: string) =>
			`TEST-API-${this.stage} ${shortDescription}`;
		...
		new GuAlarm(this, 'ApiGateway4XXAlarmCDK', {
			...
			alarmDescription: alarmDescription(
				'Test api received an invalid request',
			),
			evaluationPeriods: 1,
			threshold: 1,
			snsTopicName: 'retention-dev',
