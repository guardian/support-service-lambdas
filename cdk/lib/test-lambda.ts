...
export interface TestLambdaProps extends GuStackProps {
	stack: string;
	stage: string;
...
export class TestLambda extends GuStack {
	constructor(scope: App, id: string, props: TestLambdaProps) {
		super(scope, id, props);

		const app = 'test-lambda';
		...
		const alarmName = (shortDescription: string) =>
			`TEST-LAMBDA-${this.stage} ${shortDescription}`;
		...
		new GuAlarm(this, 'ApiGateway4XXAlarmCDK', {
			...
			alarmDescription: alarmDescription(
				'Test lambda received an invalid request',
			),
			evaluationPeriods: 1,
			threshold: 1,
			snsTopicName: 'retention-dev',
